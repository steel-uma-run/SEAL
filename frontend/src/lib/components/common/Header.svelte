...
>
		...
		<a
			href={isLoggedIn
				? profileRole === "COORDINATOR"
					? "/coordinator"
					: profileRole === "LECTURER" || profileRole === "JUDGE"
						? "/lecturer"
						: profileRole === "STUDENT"
							? "/student"
							: "/"
				: "/"}
			class="flex items-center gap-2.5 no-underline hover:opacity-80 transition-opacity"
		>
			<img
	...
	<div class="flex items-center gap-5">
		<button
			class="p-2 rounded-full transition-all duration-300 hover:scale-110 hover:shadow-md hover:cursor-pointer border {theme.darkMode
				? 'bg-zinc-800 text-orange-400 border-zinc-700'
				: 'bg-white text-orange-500 border-orange-200 hover:border-orange-500'}"
