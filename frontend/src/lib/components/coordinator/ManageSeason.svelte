...
<div class="p-6 md:p-10 max-w-[1600px] mx-auto w-full">
    ...
    <header class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 border-b pb-6 {theme.darkMode ? 'border-zinc-800' : 'border-gray-100'}">
        ...
        <div class="flex items-center gap-3 mt-4 md:mt-0">
            <button onclick={toggleSeasonModal} class="flex items-center gap-2 bg-[#f26f21] hover:bg-[#d85c14] text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm transition-all cursor-pointer border-0">
                <Plus class="w-4 h-4" />
                New Season
            </button>
        </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div class="xl:col-span-2 p-8 rounded-3xl border transition-all {theme.darkMode ? 'bg-zinc-900 border-zinc-800 shadow-[0_4px_30px_rgba(0,0,0,0.2)]' : 'bg-white border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)]'}">
            <h2 class="text-xl font-bold mb-6 {theme.darkMode ? 'text-zinc-100' : 'text-gray-800'}">Active Seasons Overview</h2>
        ...
        <div class="xl:col-span-1 p-8 rounded-3xl border transition-all {theme.darkMode ? 'bg-zinc-900 border-zinc-800 shadow-[0_4px_30px_rgba(0,0,0,0.2)]' : 'bg-white border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)]'}">
        ...
        <div class="w-full max-w-lg rounded-2xl border p-8 relative transition-all shadow-2xl {theme.darkMode ? 'bg-zinc-900 border-zinc-800 text-zinc-100' : 'bg-white border-gray-100 text-gray-800'}">
            ...
            <form onsubmit={handleCreateSeason} class="flex flex-col gap-4">
                ...
                <div class="flex gap-3 mt-4">
                    <button type="button" onclick={toggleSeasonModal} class="w-1/2 rounded-xl border py-3 font-semibold transition-all cursor-pointer {theme.darkMode ? 'border-zinc-800 bg-transparent text-zinc-300 hover:bg-zinc-800/20' : 'border-gray-250 bg-transparent text-gray-700 hover:bg-gray-50'}">
                        Cancel
                    </button>
                    <button type="submit" disabled={isSeasonLoading} class="w-1/2 bg-orange-500 hover:bg-orange-600 text-white rounded-xl py-3 font-semibold disabled:opacity-50 transition-all shadow-sm cursor-pointer border-0">
