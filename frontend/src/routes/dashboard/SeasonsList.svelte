...
<div class="bg-white/90 backdrop-blur-md p-6 rounded-2xl shadow-sm border border-gray-100 sticky top-24">
    ...
    {#if errorMessage}
        <div class="p-4 bg-red-50 text-red-600 rounded-xl text-sm border border-red-100 flex items-start gap-2">
        ...
        <ul class="space-y-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
            {#each seasons as season}
                <li class="group p-5 border border-gray-100 rounded-xl hover:border-orange-200 hover:shadow-md transition-all duration-300 bg-white relative overflow-hidden">
                    ...
                    <div class="flex flex-col gap-1.5 mt-4 pt-3 border-t border-gray-50 text-xs text-gray-400 font-medium">
                        <div class="flex items-center gap-2">
                            ...
                            <span>Start: <span class="text-gray-600 font-semibold">{season.start_time ? new Date(season.start_time).toLocaleString() : 'N/A'}</span></span>
                        </div>
                        <div class="flex items-center gap-2">
                            ...
                            <span>End: <span class="text-gray-600 font-semibold">{season.end_time ? new Date(season.end_time).toLocaleString() : 'N/A'}</span></span>
                        </div>
                    </div>
                </li>
            {/each}
