...
<div class="flex h-screen overflow-hidden font-sans transition-colors duration-300 {theme.darkMode ? 'bg-zinc-950 text-zinc-100' : 'bg-[#f8f9fa] text-gray-800'}">
    <!-- Sidebar -->
    <aside class="w-64 flex flex-col justify-between hidden md:flex h-full shrink-0 relative z-10 shadow-[2px_0_15px_rgba(0,0,0,0.02)] transition-colors duration-300 {theme.darkMode ? 'bg-zinc-900 border-r border-zinc-800' : 'bg-[#fff9f4] border-r border-[#ffe8d6]'}">
        <div>
            <!-- Logo -->
        ...
        <!-- Logout -->
        <div class="p-6 border-t {theme.darkMode ? 'border-zinc-800' : 'border-[#ffe8d6]/50'}">
    ...
    <!-- Main Content Area -->
    <main class="flex-1 flex flex-col h-full overflow-hidden transition-colors duration-300 {theme.darkMode ? 'bg-zinc-950' : 'bg-white'}">
