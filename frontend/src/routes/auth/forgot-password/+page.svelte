...
<main class="w-screen h-screen flex justify-center items-center bg-(--md-surface)">
	<div
		class="max-w-md w-full m-auto flex flex-col items-center rounded-xl p-8 border {theme.darkMode
			? 'bg-zinc-900 border-zinc-800 text-zinc-100 shadow-[0_4px_30px_rgba(0,0,0,0.2)]'
			: 'bg-white border-gray-100 text-gray-800 shadow-[0_4px_20px_rgba(0,0,0,0.03)]'}"
	>
		<a
			href="/auth/login"
			class="self-start flex items-center gap-2 mb-6 text-sm text-[#f54901] hover:underline font-semibold"
		>
			<ArrowLeft size={16} />
			Back to login
		</a>
		...
		{#if isSuccess}
			<div
				class="w-full p-4 rounded-xl border flex gap-3 mb-6 bg-emerald-50 border-emerald-100 text-emerald-800 dark:bg-emerald-950/20 dark:border-emerald-900/50 dark:text-emerald-400"
			>
			...
			<form class="w-full flex flex-col gap-4" onsubmit={handleResetPassword}>
				<div class="space-y-1">
					...
					<div class="relative w-full">
						<input
							id="email"
							class="w-full rounded-xl border p-3 pl-10 focus:ring-2 focus:ring-orange-500 transition-all outline-none {theme.darkMode
								? 'border-zinc-800 bg-zinc-950 text-zinc-100 placeholder-zinc-650'
