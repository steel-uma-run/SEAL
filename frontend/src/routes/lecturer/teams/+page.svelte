...
<div class="max-w-6xl mx-auto w-full p-4 md:p-8">
	...
	{:else if myMentoredTracks.length === 0}
		<div class="text-center py-16 border-2 border-dashed rounded-2xl {theme.darkMode ? 'border-zinc-800 text-zinc-500' : 'border-gray-200 text-gray-400'}">
			...
			<p class="text-sm mt-1">You haven't been assigned as a mentor to any tracks in the active season.</p>
		</div>
	{:else}
		<div class="space-y-8">
				...
				<div class="rounded-2xl border transition-all overflow-hidden {theme.darkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-gray-200 shadow-sm'}">
					<div class="p-6 border-b {theme.darkMode ? 'border-zinc-800' : 'border-gray-200'}">
						<div class="flex justify-between items-start mb-2">
							...
							<span class="text-xs font-bold uppercase tracking-wider px-2.5 py-1 rounded-full bg-violet-100 text-violet-700">
								Mentor
							</span>
						</div>
						<p class="text-sm {theme.darkMode ? 'text-zinc-400' : 'text-gray-500'}">
					...
					<div class="p-6">
						{#if track.teams && track.teams.length > 0}
							<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
								{#each track.teams as team}
									<div class="p-5 rounded-xl border flex flex-col h-full {theme.darkMode ? 'bg-zinc-950/50 border-zinc-800' : 'bg-gray-50 border-gray-200'}">
										<div class="mb-4 flex justify-between items-start">
											<h3 class="font-bold text-lg {theme.darkMode ? 'text-zinc-100' : 'text-gray-900'}">
												{team.name}
											</h3>
											<span class="text-xs font-semibold px-2 py-0.5 rounded-full {team.status === 'APPROVED' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}">
												{team.status}
											</span>
										</div>

										<div class="flex-grow">
											<h4 class="text-xs font-bold uppercase tracking-wider mb-2 {theme.darkMode ? 'text-zinc-500' : 'text-gray-500'}">Latest Submission</h4>
											{#if team.submissions && team.submissions.length > 0}
												{@const latestSub = team.submissions[team.submissions.length - 1]}
												<div class="p-3 rounded-lg text-sm {theme.darkMode ? 'bg-zinc-900 border border-zinc-800' : 'bg-white border border-gray-200'}">
											...
											{:else}
												<p class="text-sm italic {theme.darkMode ? 'text-zinc-600' : 'text-gray-400'}">No submissions yet.</p>
											{/if}
										</div>
									</div>
								{/each}
						...
						{:else}
							<div class="text-center py-8">
								<p class="text-sm italic {theme.darkMode ? 'text-zinc-500' : 'text-gray-400'}">No teams assigned to this track yet.</p>
							</div>
						{/if}
					</div>
				</div>
