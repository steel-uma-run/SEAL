...
<div class="max-w-4xl mx-auto w-full p-4 md:p-8">
	<a
		href="/student"
		class="inline-flex items-center gap-2 transition-colors mb-8 font-medium {theme.darkMode
	...
	{:else}
		<div class="space-y-8">
			{#each submissions as sub, index}
				...
				>
					<div
						class="flex flex-col md:flex-row justify-between items-start gap-4 mb-6 pb-6 border-b {theme.darkMode
							? 'border-zinc-800'
							: 'border-gray-100'}"
					>
						<div>
							<div class="flex items-center gap-3 mb-2">
								...
								<span
									class="text-xs font-semibold px-2.5 py-1 rounded-full {theme.darkMode
							...
							{#if sub.submitted_at}
								<div
									class="flex items-center gap-1 mt-3 text-xs {theme.darkMode
										? 'text-zinc-500'
										: 'text-gray-400'}"
								>
									<Clock class="w-3 h-3" />
						...
						<div class="shrink-0 text-right">
							{#if sub.has_been_graded}
								...
								<div class="flex items-baseline gap-1 justify-end">
									<span
										class="text-3xl font-extrabold {sub.total_score >= 50
											? 'text-green-500'
											: 'text-red-500'}"
									>
									...
									<span
										class="text-sm font-bold {theme.darkMode ? 'text-zinc-500' : 'text-gray-400'}"
										>/100</span
									>
								</div>
							{:else}
								<div class="inline-flex flex-col items-end">
									<span
										class="px-3 py-1.5 rounded-lg text-sm font-medium {theme.darkMode
					...
					{#if sub.has_been_graded && sub.scores}
						<div>
							<h3
								class="text-sm font-bold uppercase tracking-wider mb-4 {theme.darkMode
									? 'text-zinc-300'
									: 'text-gray-700'}"
							>
								Feedback & Scores
							...
							<div class="space-y-3">
								{#each sub.scores as score}
									<div
										class="p-4 rounded-xl {theme.darkMode
											? 'bg-zinc-950/50'
											: 'bg-gray-50'} flex flex-col md:flex-row justify-between gap-4"
									>
										<div>
											...
											{#if score.comment}
												<p
													class="text-sm mt-1 italic {theme.darkMode
														? 'text-zinc-400'
														: 'text-gray-600'}"
												>
													"{score.comment}"
										...
										<div class="shrink-0">
											<span
												class="inline-flex items-center justify-center min-w-[3rem] px-2 py-1 rounded-lg font-bold text-sm {theme.darkMode
