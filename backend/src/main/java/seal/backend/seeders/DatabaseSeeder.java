...
public class DatabaseSeeder implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        if (userRepository.count() == 0) {
            ...
            // Student Account
            User student = new User("Student Nguyen", Role.STUDENT, "student@fpt.edu.vn", defaultPassword);
            userRepository.save(student);

            // Judge (Lecturer) Account
            User judge = new User("Lecturer Tran", Role.JUDGE, "lecturer@fpt.edu.vn", defaultPassword);
            userRepository.save(judge);

            // Coordinator Account
            User coordinator = new User("Coordinator Le", Role.COORDINATOR, "coordinator@fpt.edu.vn", defaultPassword);
